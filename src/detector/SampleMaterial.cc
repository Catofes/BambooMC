...
using std::vector;
// anonymous namespace to register the SampleMaterial

namespace
{

    BambooMaterial *createSampleMaterial()
    {
        return new SampleMaterial("SampleMaterial");
    }

    const std::string SampleMaterialName("SampleMaterial");

    const bool registered = BambooMaterialFactory::Instance()->registerMaterial(SampleMaterialName,
...
void SampleMaterial::defineMaterials()
{
    ...
    G4Material *gasXenon = pNistManager->FindOrBuildMaterial("G4_Xe");
    materialVec.push_back(gasXenon);
    ...
    G4Material *TMA = new G4Material("Trimethylamine", tmaDensity, 3, kStateGas, 300 * kelvin, tmaPressure);
    TMA->AddElement(N, 1);
    TMA->AddElement(C, 3);
    TMA->AddElement(H, 9);
    ...
    G4Material *XeTMAMixture = new G4Material("XeTMAMixture", xe_tma_density, 2, kStateGas, 300 * kelvin,
                                              _multiple * gasXenon->GetPressure() + tmaPressure);
